...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        var newwidth;
        //statically assign a size of each pizza size
        if (size == 1) { // small pizza = 25% ie 4 wide
            newwidth = '25%';
        } else if (size == 2) { // medium pizza = 333.333% ie: 3 wide
            newwidth = '33.333%';
        } else if (size == 3) { // large pizza = 50% ie: 2 wide
            newwidth = '50%';
        } else {
            newwidth = '25%'; // this should never happen
            console.log("new pizza size which has not been programmed into the javascript");
        }
...
function updatePositions() {
    ...
    // only calculate the list of .mover class elements once to save time
    if (!window.items) {
        window.items = document.getElementsByClassName('mover');
    }
    var items = window.items; // cache window.items;

    var scrollTop = document.body.scrollTop; // cached value - makes a HUGE difference

    var phases = []; // pre-calculating the phases didn't make such a huge difference
    var i;
    for (i = 0; i < 5; i++) {
    ...
    for (i = 0; i < items.length; i++) {
        var phase = phases[i % 5]; // return just the specific phase avoiding expensive sin calculations

        // speed was around 0.3ms slower with this code, but seems to be worth it to paint faster ie: no new layout required

        var pixel = items[i].basicLeft + 100 * phase - 1250 + 'px';
...
document.addEventListener('DOMContentLoaded', function() {
    var cols = 8;
    var s = 256;
    var movingPizzas = document.getElementById("movingPizzas1"); // outside the loop
